/**
 * CaseDetailScreen Component - Android
 * File: CaseDetailScreen.android.jsx
 */
...
import {
  ...
  StyledSection,
  StyledWarningBox,
} from './CaseDetailScreen.android.styles';
...
const CaseDetailScreenAndroid = ({ caseId }) => {
  const { t } = useI18n();
  const {
    caseItem,
    citations,
    ...
    <StyledContainer testID={CASE_DETAIL_TEST_IDS.screen}>
      ...
      <StyledSection testID={CASE_DETAIL_TEST_IDS.ventilatorSettings}>
        ...
        <StyledBlock>
          {SETTING_KEYS.map((k) => (
            <Text key={k} variant="caption">
              {t(`ventilation.recommendation.settings.${k}`, { default: k })}: {formatValue(ventSettings[k])}
            </Text>
          ))}
        </StyledBlock>
      </StyledSection>
...
export default CaseDetailScreenAndroid;
