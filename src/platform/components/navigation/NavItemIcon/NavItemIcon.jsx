/**
 * NavItemIcon Component
 * Maps icon string identifiers to styled SVG icon components
 * Supports all navigation items in the application
 */
...
/**
 * NavItemIcon Component
 * Renders a properly styled icon for navigation items
 *
 * @param {Object} props - Component props
 * @param {string} props.name - Icon identifier (e.g., 'Cog', 'Building', 'User')
 * @param {string} props.size - Icon size in pixels (default: 24)
 * @param {string} props.color - Icon color (default: currentColor)
 * @param {string} props.className - Additional CSS classes
 * @param {boolean} props.isNative - Whether rendering in React Native (internal use)
 *
 * @returns {JSX.Element} Styled icon component
 */
export const NavItemIcon = ({
  ...
  // For web platforms, return SVG directly
  if (!isNative) {
    return (
      <span
        className={className}
  ...
  // For React Native, return a Text component representing the icon
  // This is a fallback; ideally React Native should use react-native-svg
  return <span style={{ fontSize: size / 2, color }}>{name}</span>;
};
