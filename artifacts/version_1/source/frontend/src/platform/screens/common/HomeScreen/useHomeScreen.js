...
export default function useHomeScreen() {
  ...
  const loadSummary = useCallback(async (facilityId) => {
    start();
    try {
      const summary = await loadHomeSummaryUseCase({
        facilityId: canSelectFacility ? facilityId || undefined : undefined,
      });
      succeed(summary);
    } catch (error) {
      fail(error?.code || 'HOME_SUMMARY_LOAD_FAILED');
    }
  }, [canSelectFacility, fail, start, succeed]);

  useEffect(() => {
    if (!canSelectFacility && selectedFacilityId) {
      setSelectedFacilityId(null);
    }
  }, [canSelectFacility, selectedFacilityId]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        const summary = await loadHomeSummaryUseCase({
          facilityId: canSelectFacility ? selectedFacilityId || undefined : undefined,
        });
  ...
  }, [canSelectFacility, fail, selectedFacilityId, start, succeed]);
