...
export default function useHistoryScreen(options = {}) {
  ...
  const selectedFacility = useMemo(() => {
    if (canSelectFacility) {
      if (!selectedFacilityOverrideId) return null;
      return facilityOptions.find((facility) => facility.id === selectedFacilityOverrideId) || null;
    }

    const selectedId = activeFacilityOption?.id || facilityOptions[0]?.id || null;
    return (
  ...
  }, [activeFacilityOption, canSelectFacility, facilityOptions, selectedFacilityOverrideId]);

  const selectedFacilityId = canSelectFacility ? selectedFacility?.id || null : null;
  ...
  const loadTracking = useCallback(async () => {
    ...
    try {
      ...
      if (canSelectFacility && selectedFacilityId) params.facilityId = selectedFacilityId;
  ...
  }, [admittedAdmissionId, canSelectFacility, detailMode, selectedFacilityId]);
  ...
  const handleFacilityClear = useCallback(() => {
    ...
    setSelectedTracking(null);
    setDetailErrorCode(null);
    setSelectedFacilityOverrideId(null);
  }, [canSelectFacility]);
  ...
  return {
    ...
    facilitySearch: {
      ...
      isAllSelected: canSelectFacility && !selectedFacility,
