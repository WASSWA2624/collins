...
export default function useAssessmentScreen() {
  ...
  const defaultFacilityOption = activeFacilityOption || assignedFacilityOptions[0] || null;
  const defaultFacilityId = defaultFacilityOption?.id || null;
  const activeFacilityLabel = defaultFacilityOption?.name || defaultFacilityId || null;
  const assignedFacilityIds = useMemo(
    () => new Set(assignedFacilityOptions.map((facility) => facility.id)),
  ...
  }, [canSelectFacility, debouncedFacilityQuery]);

  const mergedInputs = useMemo(() => {
    const normalized = normalizeInputs(inputs, clientRecordId);

    if (!canSelectFacility) {
      const normalizedFacilityId = cleanText(normalized.facilityId);
      if (normalizedFacilityId && assignedFacilityIds.has(normalizedFacilityId)) {
        return normalized;
      }
  ...
  const selectedFacilityValue = useMemo(() => {
    ...
    const assignedFacility = assignedFacilityOptions.find((facility) => facility.id === mergedInputs.facilityId);
    if (assignedFacility) return assignedFacility;
    if (mergedInputs.facilityId) {
      return normalizeFacilityOption({ id: mergedInputs.facilityId, name: mergedInputs.facilityId });
    }
    return null;
  }, [activeFacilityOption, assignedFacilityOptions, mergedInputs.facilityId, selectedFacility]);
  const displayedFacilityOptions = useMemo(
    () => uniqueFacilities(selectedFacilityValue, activeFacilityOption, assignedFacilityOptions, facilityOptions),
    [activeFacilityOption, assignedFacilityOptions, facilityOptions, selectedFacilityValue]
  );
  ...
  const updateFacilityQuery = useCallback(
    ...
    [canSelectFacility, selectedFacilityValue, updateInput]
  );
