...
export const isPlatformAdminUser = (user) =>
  getRoleKeysForUser(user).includes(MEMBERSHIP_ROLES.PLATFORM_ADMIN);
...
export const getActiveFacilityContext = (user) => {
  ...
  if (options.length > 1 && !isPlatformAdminUser(user)) {
    const [first] = options;
    return {
      ...(first.facility || {}),
      id: first.facilityId,
      ...
      membershipIds: first.membershipIds,
    };
  }
...
export const canSelectFacilityScope = (user) => isPlatformAdminUser(user);

export const getDefaultFacilityOptionForUser = (user) =>
  getActiveFacilityContext(user) || getFacilityOptionsForUser(user)[0] || null;
