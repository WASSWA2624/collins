...
describe('tracking.model', () => {
  it('normalizes backend tracking rows with facility, bed, review, sync, conflict, and missing-data labels', () => {
    ...
    expect(row.patientCode).toBe('CIT0001');
    expect(row.patientCode).toMatch(/^CIT\d{4}$/);
    expect(row.appPatientCode).toBe('CIT0001');
  ...
  it('builds searchable tracking rows from patient demographics and identifiers', () => {
    ...
    const otherRow = normalizeTrackingItem({
      ...
      admissionId: 'adm-2',
      patientId: 'patient-2',
      appAdmissionCode: 'SAPAIC0002',
      patient: {
        id: 'patient-2',
        appPatientCode: 'CIT0002',
    ...
    expect(filterTrackingRows([row, otherRow], 'CIT0002')).toEqual([otherRow]);
  });
});
