...
const flushFacilityLoad = async (minimumCalls = 1) => {
  await waitFor(() => {
    expect(mockSearchFacilitiesUseCase.mock.calls.length).toBeGreaterThanOrEqual(minimumCalls);
  });
  const latestResult = mockSearchFacilitiesUseCase.mock.results[
    mockSearchFacilitiesUseCase.mock.results.length - 1
  ];
  await act(async () => {
...
describe('auth entry screens', () => {
  ...
  it('renders a branded registration entry with sign-in navigation', async () => {
    ...
    expect(getByTestId('register-facility')).toBeTruthy();
    expect(getByTestId('register-facility-input')).toBeTruthy();
  ...
  it('submits validated registration details through the auth hook', async () => {
    ...
    fireEvent(getByTestId('register-facility-input'), 'focus');
    await waitFor(() => expect(getByTestId('register-facility-option-0')).toBeTruthy());
    fireEvent.press(getByTestId('register-facility-option-0'));
    await flushFacilityLoad(2);
