...
import {
  buildAdmissionIdentifierCode,
  buildNewPatientReadiness,
  buildClinicalSummary,
  buildCurrentReadingsProgressAssessment,
  buildPatientIdentifierCode,
  normalizeAppRecordCode,
} from '../src/modules/newPatients/newPatients.service.js';

test('patient and admission codes use facility, patient name, and sequence values', () => {
  assert.equal(buildPatientIdentifierCode({
    facilityName: 'Hospital International',
    sequence: 1,
  }), 'HOS0001');

  assert.equal(buildAdmissionIdentifierCode({
    patient: { firstName: 'John', lastName: 'Doe' },
    facilityName: 'International Hospital',
    sequence: 1,
  }), 'JODOIH0001');

  assert.match(buildAdmissionIdentifierCode({
    patient: { firstName: 'Ab' },
    facilityName: 'Icu',
    sequence: 2,
  }), /^AB[A-Z0-9]{2}IC0002$/);

  assert.equal(normalizeAppRecordCode(' ab12cd '), 'AB12CD');
  assert.equal(normalizeAppRecordCode('COL-P-123'), 'COLP123');
