...
const buildFacilityWhere = (type, scope = {}) => {
  const scopedFacilityId = typeof scope === 'string' ? scope : scope.facilityId;
  const scopedFacilityIds = Array.isArray(scope.facilityIds) ? scope.facilityIds : [];
  const facilityFilter = scopedFacilityId
    ? scopedFacilityId
    : scopedFacilityIds.length > 0
      ? { in: scopedFacilityIds }
      : null;
  ...
  if (type === 'sync-conflict') return { facilityId: facilityFilter };
  return { admission: { facilityId: facilityFilter } };
};
...
export const listReviewQueue = async (userId, { facilityId, entityType, page, limit }) => {
  const facilityScope = await resolveFacilityAccessScope(userId, facilityId, REVIEW_ROLES);
  ...
  for (const type of entityTypes) {
    ...
    const where = {
      ...
      ...buildFacilityWhere(type, facilityScope),
    };
