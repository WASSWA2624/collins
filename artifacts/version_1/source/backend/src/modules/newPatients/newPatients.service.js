...
const GENERATED_RECORD_CODE_MAX_LENGTH = 32;
const PATIENT_CODE_FACILITY_PREFIX_LENGTH = 3;
const PATIENT_CODE_SEQUENCE_WIDTH = 4;
const ADMISSION_CODE_PATIENT_PREFIX_LENGTH = 4;
const ADMISSION_CODE_FACILITY_PREFIX_LENGTH = 2;
const ADMISSION_CODE_SEQUENCE_WIDTH = 4;
const LEGACY_APP_RECORD_CODE_LENGTH = 6;
const APP_RECORD_CODE_ALPHABET = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';

const randomCodeCharacter = () => APP_RECORD_CODE_ALPHABET[randomInt(APP_RECORD_CODE_ALPHABET.length)];
...
const normalizeLetterToken = (value) => normalizeCodeToken(value).replace(/[^A-Z]/g, '');

const splitLetterWords = (value) =>
  String(value ?? '')
    .normalize('NFKD')
    .replace(/[\u0300-\u036f]/g, '')
    .toUpperCase()
...
const padCodeSection = (value, length, filler = randomCodeCharacter) => {
  let token = String(value || '').slice(0, length);
  while (token.length < length) token += filler();
  return token;
};

export const formatIdentifierSequence = (sequence, width = PATIENT_CODE_SEQUENCE_WIDTH) => {
  const value = Number(sequence);
  const safeValue = Number.isFinite(value) && value > 0 ? Math.trunc(value) : 1;
  return String(safeValue).padStart(width, '0');
};

export const buildFacilityPatientCodePrefix = (facilityName) =>
  padCodeSection(normalizeLetterToken(facilityName), PATIENT_CODE_FACILITY_PREFIX_LENGTH, () => 'X');

export const buildPatientAdmissionCodePrefix = (patient = {}) => {
  const firstName = normalizeLetterToken(patient.firstName);
  const lastName = normalizeLetterToken(patient.lastName);
  const optionalNameWords = splitLetterWords(patient.optionalName);

  if (firstName && lastName) {
    return padCodeSection(
      `${firstName.slice(0, 2)}${lastName.slice(0, 2)}`,
      ADMISSION_CODE_PATIENT_PREFIX_LENGTH
    );
  }

  if (!firstName && optionalNameWords.length >= 2) {
    return padCodeSection(
      `${optionalNameWords[0].slice(0, 2)}${optionalNameWords[1].slice(0, 2)}`,
      ADMISSION_CODE_PATIENT_PREFIX_LENGTH
    );
  }

  return padCodeSection(
    firstName || optionalNameWords[0] || normalizeLetterToken(patient.optionalName),
    ADMISSION_CODE_PATIENT_PREFIX_LENGTH
  );
};

export const buildFacilityAdmissionCodePrefix = (facilityName) => {
  const words = splitLetterWords(facilityName);
  const prefix = words.length > 1
    ? `${words[0].charAt(0)}${words[1].charAt(0)}`
    : (words[0] || normalizeLetterToken(facilityName)).slice(0, 2);
  return padCodeSection(prefix, ADMISSION_CODE_FACILITY_PREFIX_LENGTH, () => 'X');
};

export const buildPatientIdentifierCode = ({ facilityName, sequence }) =>
  `${buildFacilityPatientCodePrefix(facilityName)}${formatIdentifierSequence(sequence, PATIENT_CODE_SEQUENCE_WIDTH)}`;

export const buildAdmissionIdentifierCode = ({ patient, facilityName, sequence }) =>
  `${buildPatientAdmissionCodePrefix(patient)}${buildFacilityAdmissionCodePrefix(facilityName)}${formatIdentifierSequence(sequence, ADMISSION_CODE_SEQUENCE_WIDTH)}`;

export const createAppRecordCode = () => Array.from(
  { length: LEGACY_APP_RECORD_CODE_LENGTH },
  randomCodeCharacter
).join('');

export const normalizeAppRecordCode = (value) => {
  const token = normalizeCodeToken(value);
  return token.length > 0 && token.length <= GENERATED_RECORD_CODE_MAX_LENGTH ? token : null;
};
...
const resolveFacilityIdentifierContext = async (tx, facilityId) => {
...
const incrementFacilityIdentifierSequence = async (tx, facilityId, sequenceField) => {
  const sequence = await tx.facilityIdentifierSequence.upsert({
    where: { facilityId },
    create: {
      facilityId,
      patientSequence: sequenceField === 'patientSequence' ? 1 : 0,
      admissionSequence: sequenceField === 'admissionSequence' ? 1 : 0,
    },
    update: {
...
const createPatientCode = async (tx, facility) => buildPatientIdentifierCode({
  facilityName: facility.name,
  sequence: await incrementFacilityIdentifierSequence(tx, facility.id, 'patientSequence'),
});

const createNewPatientCode = async (tx, facility, patient) => buildAdmissionIdentifierCode({
  patient,
  facilityName: facility.name,
  sequence: await incrementFacilityIdentifierSequence(tx, facility.id, 'admissionSequence'),
});
...
export const listNewPatients = async (userId, { facilityId, status, reviewStatus, patientPathway, page, limit }) => {
  const scope = await resolveFacilityAccessScope(userId, facilityId);
  const where = {
    ...(scope.facilityId
  ...
  const referenceRangesByFacility = new Map();
  await Promise.all([...new Set(items.map((item) => item.facilityId).filter(Boolean))].map(async (itemFacilityId) => {
    referenceRangesByFacility.set(
      itemFacilityId,
      await resolveDecisionSupportReferenceRanges(itemFacilityId, { allowDevelopmentFallback: false })
    );
  }));
  ...
  return {
    items: items.map((admission) => ({
      ...
      clinicalSummary: buildClinicalSummary(admission, {
        referenceRanges: referenceRangesByFacility.get(admission.facilityId),
      }),
    })),
    total,
    page,
...
export const createNewPatient = async (payload, createdByUserId, auditContext = {}) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const facility = await resolveFacilityIdentifierContext(tx, facilityId);
    const patientData = preparePatientData(admissionPayload.patient);
    const createdPatient = await tx.patient.create({
      data: {
        ...patientData,
        facilityId,
        appPatientCode: await createPatientCode(tx, facility),
      },
    });
    ...
    const admission = await tx.admission.create({
      data: stripUndefined({
        ...
        facilityId,
        appAdmissionCode: await createNewPatientCode(tx, facility, createdPatient),
