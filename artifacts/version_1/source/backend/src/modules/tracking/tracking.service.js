...
import { assertAdmissionAccess, resolveFacilityAccessScope } from '../../utils/authorization.js';
import {
  newPatientInclude,
...
export const buildTrackingAdmissionWhere = ({ facilityId, facilityIds, status, reviewStatus, patientPathway } = {}) => {
...
export const getTrackingAdmission = async (userId, admissionId) => {
  const admissionAccess = await assertAdmissionAccess(userId, admissionId);
  const admission = await prisma.admission.findUnique({
    where: { id: admissionAccess.id },
    include: fullNewPatientInclude,
  });
