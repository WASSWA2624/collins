...
Run these phases in order. Every phase starts by inspecting the existing routes, validators, controllers, services, Prisma queries, tests, and shared helpers. Reuse compliant code and clean up only the area being implemented. When frontend work in the matching phase needs backend support, verify or implement the backend route, validator, controller, service, repository/helper, Prisma schema, migration, Zod validation, auth/RBAC, facility isolation, audit logging, offline idempotency, conflict handling, and backend tests in the same phase.
