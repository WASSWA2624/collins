...
facilitiesRouter.post('/', validateRequest(createFacilitySchema), addFacility);
facilitiesRouter.get('/:id', validateRequest(facilityIdSchema), getFacility);
facilitiesRouter.post('/:id/request-verification', validateRequest(facilityIdSchema), requestVerification);
facilitiesRouter.get('/:id/equipment-profile', validateRequest(facilityIdSchema), getEquipmentProfile);
facilitiesRouter.patch('/:id/equipment-profile', validateRequest(updateEquipmentProfileSchema), patchEquipmentProfile);
facilitiesRouter.post('/:id/memberships/request', requireAuth, validateRequest(membershipRequestSchema), createMembershipRequest);
facilitiesRouter.patch('/:id/memberships/:membershipId', requireAuth, updateMembership);
