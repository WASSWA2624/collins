...
import { buildTrackingPrintDocument } from './trackingPrintData';
import { HISTORY_TEST_IDS } from './types';

const renderPrintFields = (fields = []) => {
  if (fields.length === 0) return null;
  return (
    <StyledPrintGrid>
      {fields.map((item, index) => (
        <StyledPrintField key={`${item.label}-${index}`}>
          <StyledPrintFieldLabel>
...
const renderPrintRecordSection = (section) => {
  if (!section?.items?.length) return null;
  return (
    <StyledPrintSection key={section.id} data-print-section="true">
      <StyledPrintSectionTitle>
        <Text variant="label">{section.title}</Text>
      </StyledPrintSectionTitle>
      {section.items.map((item) => (
        <StyledPrintSubsection key={item.id} data-print-section="true">
          <StyledPrintSubsectionTitle>
            <Text variant="caption">{item.title}</Text>
          </StyledPrintSubsectionTitle>
          {renderPrintFields(item.fields)}
        </StyledPrintSubsection>
      ))}
    </StyledPrintSection>
  );
...
const renderPrintDocument = (printDocument) => {
  if (!printDocument) return null;

  return (
    <StyledPrintDocument
      data-print-document="true"
      data-testid={HISTORY_TEST_IDS.printDocument}
      testID={HISTORY_TEST_IDS.printDocument}
    >
      <StyledPrintDocumentHeader>
        <StyledPrintTitle>
          <Text variant="h2">{printDocument.title}</Text>
        </StyledPrintTitle>
        <StyledPrintSubtitle>
          <StyledPrintMeta>
            <StyledPrintMetaItem>
              <Text variant="caption">{printDocument.patientLabel}</Text>
            </StyledPrintMetaItem>
            <StyledPrintMetaItem>
              <Text variant="caption">{printDocument.bedLabel}</Text>
            </StyledPrintMetaItem>
            {printDocument.facilityName ? (
              <StyledPrintMetaItem>
                <Text variant="caption">{printDocument.facilityName}</Text>
              </StyledPrintMetaItem>
            ) : null}
          </StyledPrintMeta>
          <StyledPrintMetaItem>
            <Text variant="caption">{printDocument.generatedAtLabel}</Text>
          </StyledPrintMetaItem>
        </StyledPrintSubtitle>
      </StyledPrintDocumentHeader>

      {printDocument.patientRows.length > 0 ? (
        <StyledPrintSection data-print-section="true">
          <StyledPrintSectionTitle>
            <Text variant="label">Patient details</Text>
          </StyledPrintSectionTitle>
          {renderPrintFields(printDocument.patientRows)}
        </StyledPrintSection>
      ) : null}

      {printDocument.sectionGroups.map((section) => (
        <StyledPrintSection key={section.id} data-print-section="true">
          <StyledPrintSectionTitle>
            <Text variant="label">{section.title}</Text>
          </StyledPrintSectionTitle>
          {section.groups.map((group) => (
            <StyledPrintSubsection key={`${section.id}-${group.title}`}>
              <StyledPrintSubsectionTitle>
                <Text variant="caption">{group.title}</Text>
              </StyledPrintSubsectionTitle>
              {renderPrintFields(group.fields)}
            </StyledPrintSubsection>
          ))}
        </StyledPrintSection>
      ))}

      {printDocument.captureSections?.map(renderPrintRecordSection)}

      {printDocument.timeline.length > 0 ? (
        <StyledPrintSection data-print-section="true">
          <StyledPrintSectionTitle>
            <Text variant="label">Clinical event timeline</Text>
          </StyledPrintSectionTitle>
          <StyledPrintTimeline>
            {printDocument.timeline.map((entry) => (
              <StyledPrintTimelineItem key={entry.id}>
                <StyledPrintTimelineTitle>
                  <StyledPrintTimelineText>
                    <Text variant="caption">{entry.title}</Text>
                  </StyledPrintTimelineText>
                  {entry.occurredAt ? (
                    <StyledPrintTimelineText>
                      <Text variant="caption">{entry.occurredAt}</Text>
                    </StyledPrintTimelineText>
                  ) : null}
                </StyledPrintTimelineTitle>
                {renderPrintFields(entry.fields)}
              </StyledPrintTimelineItem>
            ))}
          </StyledPrintTimeline>
        </StyledPrintSection>
...
const renderDetailPanel = ({
  ...
  const canRenderPrintDocument = typeof document !== 'undefined';
  const printDocument = canRenderPrintDocument && row
    ? buildTrackingPrintDocument({ tracking: selectedTracking, t })
    : null;
  ...
  return (
    ...
    >
      <StyledScreenOnlyDetail data-print-hidden="true">
        <StyledItemRow>
          <StyledItemMain>
            <Text variant="label">
              {row
        ...
        <StyledHeader data-print-hidden="true">
          <StyledHeaderCopy>
            ...
            <Text variant="body" color="text.secondary">
              {facilitySearch.isAllSelected
                ? t('ventilation.tracking.facility.all')
  ...
  return (
    ...
    >
      ...
      <StyledControlsRow data-print-hidden="true">
        ...
        {facilitySearch.canSelect ? (
          <StyledFacilityFilter
            data-testid={HISTORY_TEST_IDS.facility}
            testID={HISTORY_TEST_IDS.facility}
          >
            <FacilitySearchSelect
              placeholder={facilitySearch.isAllSelected
                ? t('ventilation.tracking.facility.all')
                : t('ventilation.tracking.facility.placeholder')}
              ...
              clearLabel={t('ventilation.tracking.facility.all')}
