...
describe('datasetCapture.model', () => {
  it('parses pasted ICU notes into structured editable preview fields', () => {
    const draft = parseDatasetCaptureNote(
      'Patient name Jane, hospital number H123. Age 54 male. SpO2 92 FiO2 40% RR 28 pH 7.31 PaO2 70 PaCO2 50 HCO3 24 lactate 2.1 mode VC VT 450 set RR 20 PEEP 8 plateau 24 peak 30.'
    );
  ...
  it('builds submission payloads without raw notes or patient identifiers', () => {
    ...
    expect(submission.sourceType).toBe('pasted_note_capture');
